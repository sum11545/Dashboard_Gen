...
Each card must include the KPI title KPI value.ChartSection:Render 5 charts in a responsive grid layout.Each chart should be inside a card adjust size according  to chart with a title chart canvas and a short insight below.UIRequirements:Use a modern glassmorphism UI design with a stylish background color using ${randomColor}
Use modern web fonts like Poppins Inter or Roboto.Ensure a clean responsive and elegant layout using card or grid-based structure.Apply subtle shadows rounded corners and hover effects to chart cards and buttons.Ensure all elements are visually consistent with no overlap or clutter.Maintain good spacing and readability across all screen sizes make dont.Make sure the layout doesn't shift left and keeps the charts readable on all screen sizes
