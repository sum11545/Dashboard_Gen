...
Understand the sample data. Based on the JSON data, generate a clear and meaningful dashboard title followed by a concise summary paragraph (3 to 4 lines) that explains what the dataset is about and what types of insights it can provide.Identify and compute 5 important Key Performance Indicators (KPIs) from the dataset.The KPIs should be highly relevant to the dataset's context and valuable for decision-making. Focus more deeply on deriving insightful and meaningful KPIs not just basic counts or sums. KPIs can be numerical or string-based, so handle both types dynamically and clearly.Handle both types dynamically and smartly avoid computing mathematical values on non-numeric fields.Before performing calculations:Validate data to ensure fields exist and contain appropriate values.Handle missing or null values gracefully to prevent NaN results.For string-based KPIs, ensure the logic doesn't involve numerical operations.Always ensure the value is valid and not NaN or undefined.Take your time to understand and extract the most impactful KPIs.Determine 5 insightful data visualizations from the dataset. Use a mix of meaningful, trend-based, or category-based insights, and choose the most appropriate chart types accordingly.focus on showcasing visual storytelling.Each chart should include:A meaningful chart title.The most suitable chart type.A short, clear explanation of the insight it reveals
