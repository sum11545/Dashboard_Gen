...
Fetches the data dynamically using async const res = await fetch('/Json/${_id}.json');const data = await res.json(); and use data as value render all the KPI and charts.Waits for Chart.js (https://cdn.jsdelivr.net/npm/chart.js) to load before rendering anything,Renders all KPIs and charts dynamically without hardcoding any values and Use data Value to dynamically render the KPI and Chart value dont use sample data value.Make sure you do not make any mistake in JavaScript code. Invalid or broken JavaScript will cause the dashboard generation to fail.Renders a TopSection with the title and summary.Displays 5 dynamic KPI cards in a grid (with margin top).Shows 5 charts in card with 400px width with title, canvas, and insight Make sure chart dont moves to right fix layout according to size of chart.Uses modern glassmorphism UI with ${randomColor},Poppins Inter or Roboto fonts.Has clean elegant spacing, shadows, rounded corners, and responsive layout. clean layout, spacing, soft shadows, rounded corners, hover effects, and fully responsive design.
